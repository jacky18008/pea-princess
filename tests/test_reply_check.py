...
class TestAuthority(unittest.TestCase):
    ...
    def test_negated_viewing_decision_is_not_permission(self):
        self.assertEqual([], [f for f in RC.scan("B 不該被當成可排看房。") if f["kind"] == "decision"])
        self.assertEqual([], [f for f in RC.scan("B 不能視為已排看房。") if f["kind"] == "decision"])
        self.assertEqual([], [f for f in RC.scan("B is not booked for a viewing.") if f["kind"] == "decision"])
        mixed = [f for f in RC.scan("A 目前可排看房；B 不該被當成可排看房。") if f["kind"] == "decision"]
        self.assertEqual(1, len(mixed), mixed)
