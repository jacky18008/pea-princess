...
## The role pipeline contracts
`references/pipeline.md` splits a run into planner, executors, verifier and integrator, and three schemas carry the files between them. `plan-schema.json` is what the planner writes: which axes to work, the script calls as argument templates, the paste requests and the fixed-question ids the tier owes — what to GET, never which fields survive, and `scripts/plan.py --check` fails if the planner dropped anything the scaffold marked required. `evidence-schema.json` is what the executors write, one item per fact — `{id, axis, claim, value, unit, source, quote, fetched_at, script, computed_by}` for something read, or `status: unknown` with a `tried` list for something nobody could get — and never a verdict, a score or a sum that is not a recorded `calc.py` call. `verified-schema.json` is what the verifier writes: `pass` / `fail(reason)` / `unknown` per item plus a `replan` list capped at one round, produced by `scripts/verify.py` first and adjusted by a model only on the items it flagged. The integrator may use verified items only; anything else is unknown in the report. The referent word lists are shared between `scripts/verify.py` and `bench/grade.py` through `scripts/referents.py`, and a test asserts the two agree, because a rule that exists twice drifts.
