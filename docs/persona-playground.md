...
預設啟動會開啟持續運作的本機開發監督程序，監看 Git 索引選定的公開包與控制器來源，包含已追蹤但未提交的修改。來源穩定後，自動建置到私人暫存目錄、驗證並凍結 ZIP 與同次 prompt pack；不覆寫 repo 的 `dist`。只有模型 worker 完全閒置才切換伺服器。正在執行的呼叫不會被中斷；來源改變、建置失敗或切換中時，新呼叫會被阻擋，不會暗中沿用舊版。啟動、更新和重整不會自動呼叫模型。

回答者讀取 ZIP 解開的 `skills/pea-princess/`，保留 ZIP 與逐檔 SHA-256，不使用電腦上另外安裝的私人 skill。歷史對話與匯出保留原版本；測新版時建立新對話，或使用「用新版重測」沿用已輸入的問題與附件。新增但尚未加入 Git 的程式不會自動帶入。這是指定 skill 版本的本機測試，不代表所有 agent 的原生安裝已驗收。

服務在啟動命令結束後繼續執行；以同一組路徑與 port 加 `--service status`、`--service logs`、`--service stop` 查看或停止。停止也會等待 worker 結束，不重試任何呼叫；電腦重開機或登出後需重新啟動服務。完整操作、來源檢查與程序失敗邊界見 [自動開發同步](playground-dev-sync.md)。

需要固定歷史 A/B 時，明確傳入 `--skill-archive /完整路徑/pea-princess-skill.zip`：沿用單次前景伺服器，驗證安全性與套件完整性，不監看來源、不自動換版。`--freeze-only` 只產生快照；此模式未指定 archive 時，仍要求預先建好的 `dist/pea-princess-skill.zip` 與目前來源相符。
