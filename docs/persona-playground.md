...
之後 Claude 或其他工作修改主 repo，不會中斷這份快照的對話。需要測新版時重新啟動，再開新對話或使用「用新版重測」沿用已輸入的問題與附件；舊紀錄不改寫，也不會改標成新版。控制器程式仍採 Git 追蹤檔案的目前內容，另保留舊 prompt pack 供相容性識別；公開包模式的回答者不使用該 pack。新增但尚未加入 Git 的程式不會自動帶入。這是指定 skill 版本的測試，仍有本機測試台的對話／工具設定，不代表所有 agent 的原生安裝已驗收。
