...
| Grok Bot, earlier package | **Synthetic six-turn episode not accepted; separate real-source episode not accepted** (`91cf2ed`, 2026-09-15) | package integrity verified directly in the Bot's computer; the six scripted consent turns used pasted A/B/C sheets and did not call live registers; a separate Greenwich A/B run did fetch some open-register evidence | Consent was overstated twice in the synthetic journey (`可考慮` → `可排`; unanswered offer → `已拒`). The real-source run saved plain comparison files, but its current journal had only one requirement, empty tasks/decisions and three pending requests. This package did not contain `doctor.py`; that absence is version drift, not a failed network check. Checker use and exact model/effort/tokens are unverified. |
| Grok Bot, integrated package | **Install and register access verified; rental quality still under test** (`aa541676`, 2026-09-15) | ZIP and installed skill hashes matched in a fresh Bot's computer. Full `python3 scripts/doctor.py --json` ran twice on the same package and host: first **20/21** (exit 1), then **21/21** (exit 0); the second raw JSON and exit code were preserved. | Nine other open-register checks succeeded on both attempts; Defra noise at the fixed probe took 25 s and returned no value first, then produced a road Lden value in 1.5 s. This is a source-specific intermittent observation whose cause is unproven, not an all-network sandbox denial. A passed doctor verifies register reachability, not per-reply skill loading, correct bedroom-level noise inference, durable project state or automatic pre-send checker execution. |
