...
Since 2026-09-16 a numeric *complete* monthly cost while a required input is unknown (council tax band not known, a heat-network tariff not priced) is an **error** even without `--strict`: the schema expects `null` there with `known_subtotal_*` beside it, because a total that silently counts an unknown as £0 is not an estimate but a claim nobody can check.
...
```
seed.py export --profile profile.yaml [--name "quiet, high, morning sun"]        # taste only: never a place or a date
