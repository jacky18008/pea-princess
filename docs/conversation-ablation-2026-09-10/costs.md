...
實驗來源凍結於 `e001e06d01a3963b4be5f7cbf03c8bc543620fde`。執行期間的期限改版、失敗與來源限制見[實驗進度與版本紀錄](../conversation-ablation-2026-09-10/conversation-ablation-progress.md)及[輸入邊界稽核](../conversation-ablation-2026-09-10/conversation-ablation-input-boundary.md)。本篇是核對後的公開統計衍生文件；原始對話、收據正文、操作映射與逐檔私有證據仍保留於私有紀錄。條件與呼叫 ID 為合成實驗標記，並非使用者身分。
...
成本統計沒有讀取品質評分欄位或回答正文，不包含原始工具輸出正文、主機檔案路徑、私人內容或帳戶資訊。完整私有核對紀錄供維護者追溯，本篇僅保留合成條件 ID、彙總與解讀所需的小計。輸入雜湊一致能證明所核對檔案未變，不能證明模型只讀了指定資料；來源邊界、部分盲評與額外輸入的限制仍適用，詳見[輸入邊界稽核](../conversation-ablation-2026-09-10/conversation-ablation-input-boundary.md)。原始資料仍保持私有。
