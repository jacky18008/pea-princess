...
本機 alpha 現在可測。以一名工程師、文字與提供資料的 MVP 粗估，少數受邀測試版還需約 1–2 週，小型公開 beta 約 3–6 週；不是承諾排程，亦不包含任意真實網站查詢或自動付款／聯絡。
...
- [ ] **受邀服務：** 身分登入、每人資料隔離、受控遠端 worker、每人配額、重啟／取消語義、刪除／匯出資料、支援流程。不能把本機 Codex 的登入分享給網頁使用者。
- [ ] **公開服務：** 多 worker 可恢復 queue、濫用與成本監控、服務指標、模型／工具權限隔離；將來源讀取與具有外部副作用的工具分開。
