...
`tools/start_playground.py` copies allowlisted tracked controller files and the explicitly named generated prompt instructions into a private version snapshot. It additionally pins the selected public `pea-princess` ZIP and safely extracts the actor package at `skills/pea-princess/`. The runtime verifies archive and per-file hashes; a frozen runtime cannot fall back to development or installed skills. Legacy `skills/vet-flat/` remains a controller dependency, not the public actor package. It excludes unrelated/untracked files, rejects unsafe source file types and verifies source stability before startup. Filename filters are not secret detection; the snapshot manifest records exact bytes, not a claim that the working tree or selected archive is a reviewed release.
...
The browser now creates `output_mode: agent` sessions. These display the model's own prose and optional choices for evaluation, without a host-generated comparison or automatic correctness approval. They skip independent post-answer page capture, and retain raw replies and tool observations. The actor may read the shipped skill package and use public read-only tools subject to current source restrictions; this is a prompt/host policy, not filesystem isolation.

New agent sessions also use the [bounded intent guard](skills/vet-flat/references/intent-guard.md). The host separates actual form answers from model-authored question prefixes, derives supported quietness/daylight strengths from user messages, and rejects stale or contradictory replies before publication. Rejected raw output and usage remain inspectable without automatic model repair. This finite grammar is not a universal semantic or prompt-injection defense and does not certify rankings, TODOs or source truth. Downloading the skill alone does not install equivalent enforcement in another agent host.

The agent lane additionally accepts explicit local-file uploads and local paths. The server snapshots bounded regular files under private, ignored attachment storage, materializes verified session copies and binds metadata to the model-call request. Names and paths are passed as data; source bytes are not served as executable browser content. Upload does not dispatch a model. The actor may read these selected copies but must not execute uploaded code/macros or obey instructions inside documents. Live research uses the CLI workspace-write sandbox with network enabled so shipped scripts can query permitted public sources and write temporary caches. `VETFLAT_CACHE` points into the temporary call directory, which is removed afterward. Package/snapshot modification and unrelated private reads remain prohibited by instructions; this is not a guarantee of same-user filesystem isolation. See [file-input boundaries](docs/playground-attachments-2026-09-11.md).

Legacy `output_mode: checked` live sessions accept only candidate-discovery proposals from the actor. The host captures sources before publishing a formal comparison, then runs `live_eligibility.py` and generates the comparison/ranking/tasks from one validated artifact. This limits actor control over formal claims; it does not prove the limited phrase parser understood every user condition or that an advert is truthful. Unknown intent and evidence remain visible. Input arriving during a call invalidates its pending comparison immediately. Read/export revalidates current input, source bytes, metadata and implementation fingerprints; historical replies remain in the record. The downloaded script requires explicit integration by other hosts. See [the adapter contract](skills/vet-flat/references/live-eligibility.md).
