...
`tools/start_playground.py` copies allowlisted tracked runtime files and the explicitly named generated prompt instructions into a private version snapshot. It excludes unrelated/untracked files, rejects unsafe source file types and verifies source stability before startup. Filename filters are not secret detection; the snapshot manifest records exact bytes, not a claim that the working tree is a reviewed release.
