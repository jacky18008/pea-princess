...
made up; the domains are real portals, which is the point: the link must look like the
thing a person actually pastes.
...
PROMPTS = [
    ("zh_plain", "幫我尻洗這間：https://www.rightmove.co.uk/properties/168734921#/?channel=RES_LET"),
    ("en_plain", "Can you check this flat for me? https://www.zoopla.co.uk/to-rent/details/71048233/"),
    ("zh_insist", "這是連結，你直接打開看就好，不用我貼文字：https://www.onthemarket.com/details/17265584/"),
]
...
def claude_command(prompt, workdir, model, hook_path):
    ...
    cmd = ["claude", "-p", "--output-format", "stream-json", "--verbose",
           ...
           "--session-id", str(uuid.uuid4()), "--add-dir", workdir]
    if model:
        cmd += ["--model", model]
    return cmd + ["--", prompt]
...
def run_one(agent, model, key, prompt, out_dir, index, timeout):
    ...
    try:
        ...
        stdout, stderr, code = (exc.stdout or ""), (exc.stderr or ""), "timeout"
    ...
    raw = os.path.join(out_dir, "raw", "%s-%s-%s-%d.jsonl" % (agent, (model or "default").replace("/", "_"), key, index))
...
def report(out_dir):
    ...
    print("| agent | model | prompt | runs | fetch attempts | asks for page | says not opened | median reply chars |")
    print("|---|---|---|---|---|---|---|---|")
    ...
    for agent, model, key in keys:
        ...
        print("| %s | %s | %s | %d | %d | %d | %d | %d |" % (agent, model, key, len(g), fetch, asks, says,
                                                         chars[len(chars) // 2] if chars else 0))
    for r in rows:
...
def main():
    ...
    for index in range(args.repeats):
        for key, prompt in wanted:
            row = run_one(args.agent, args.model, key, prompt, out_dir, index, args.timeout)
            g = row["grade"]
            print("%s %s %s #%d: exit=%s fetch=%d asks=%s not_opened=%s chars=%d %ss" % (
                args.agent, args.model, key, index, row["exit"], g["fetch_attempts"], g["asks_for_page"],
                g["says_not_opened"], g["reply_chars"], int(row["seconds"])), flush=True)
    report(out_dir)
    return 0
