...
3. Before deciding you have no file access, look for `profile.yaml` in the working folder: if it is there and you can edit files, write the change into it now (the person asked for it), then run `python3 scripts/profile_check.py profile.yaml`. Never claim a file was saved or validated without observing it. Without file tools, keep the updated requirements in the conversation and state that limitation only when saving matters; do not hand a new renter a command to run.
