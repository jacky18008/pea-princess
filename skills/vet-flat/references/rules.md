...
- **Research budget**: one `scripts/area_scan.py` call per street, then write; the single-register scripts are for one flat's own checks. Web search only for a named document after the scripts, at most three queries a reply, never the same document twice; a failed fetch is reported, not retried. A file already in context is not read again.
