...
- **Research budget**: scripts first (`scripts/area_scan.py` for a street); web search only for a named document after them, at most three queries a reply, never the same document twice; a failed fetch is reported, not retried. A file already in context is not read again.
...
- **Arithmetic is never done in your head**: use `scripts/calc.py`; without a shell, show formula and steps (weekly rent = monthly × 12 ÷ 52; deposit cap = 5 × weekly, 6 × at ≥£50k/year), and keep `computed_by: shown formula` inside `report.json`, never in the reply.
- **Escalation**: start at `standard`; `breadth` only for the final two or three flats or CONDITIONAL/EDGE over 40% unknown.
- **Legal scope**: identify the agreement first (axis 07). England assured-tenancy reforms apply from **2026-05-01**; halls, licences and lodgers differ. For in-scope monthly tenancies: no rent before signing; normally one month between signing and start. Deposit cap **five weeks, six at £50,000/year**, holding deposit one week; cite the source.
