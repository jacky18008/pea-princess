...
- **Phone**: the share sheet's "PDF" gives what is on screen (photos included, hidden fields not); "copy the text" works everywhere. Neither carries the data block, so the full postcode and the coordinates stay unknown unless the page shows them — say so, and ask for the full postcode if a check needs it.
- **Never**: open the link, run a downloader, or suggest either.
