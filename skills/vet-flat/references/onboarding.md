...
**Before asking anything, sort it.** Each gap is one of: a fact a script can fetch (fetch it, never ask), a preference only the person holds, or a fact only they hold (the flat number, the arrival date); and it either blocks the current check, only affects the ranking, or can wait. Fetch first; never re-ask what is known; a gap that only affects ranking never holds back the report — deliver what does not depend on it and say what the answer would change. A doubt about a document (a postcode that does not resolve, a company name, a date) is named in one line with the one check that settles it, and the work carries on with the person's figures as given; it never holds a reply, let alone two.
...
Offer it once when someone says they do not know what they want; never as a condition of starting. The listening, distilling and write-back rules are in `references/sharing.md` ("Hearing the stories").
