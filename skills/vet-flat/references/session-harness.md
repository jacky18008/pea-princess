...
Before ranking candidates or saving a current recommendation/TODO, use [eligibility-api.md](eligibility-api.md) with the host's current normalized conditions, evidence and independently pinned hashes/revision. For a commute comparison, first audit saved TfL receipts with `scripts/commute_compare.py` as routed by [comparison-research.md](comparison-research.md); the eligibility gate checks normalized conditions, not journey endpoint or source consistency. Recompute after changes; known mandatory failures stay blocked unless an explicit scoped user exception actually applies. The host separately checks authoritative state before and after the call and validates the returned recommendation/TODO. This checks already-normalized conditions, not automatic intent extraction, source truth or an overall PASS.

Before claiming durable storage, use `scripts/save_gate.py` for the claimed scope: `--scope comparison` checks only the bound revision/review/context export; `--scope comparison-full --evidence <current-evidence.json> --fidelity-document-id <registered-id>` checks registered source bytes, all journey alternatives and concrete pending checks as in [comparison-fidelity.md](comparison-fidelity.md); `--scope report --output-id <registered-output-id>` additionally requires a current snapshotted output and completed task. Its revision/hash receipt checks recorded state, not whether the caller interpreted the person's words correctly. If the tool did not run or failed, do not say the requested work is saved. Without shell access, use the manual fallback below and label machine verification unavailable.

For a concrete candidate tradeoff or viewing hold, use [boundary-turn.md](boundary-turn.md) and [boundary-api.md](boundary-api.md). They record scoped acceptance, refusal, retention or an explicit global change in this same state and derive comparison/viewing work together. Acceptance of an estimated journey does not verify its duration. Candidate-specific permission does not change the global requirement.
