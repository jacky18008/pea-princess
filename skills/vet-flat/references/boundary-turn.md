...
Before saying the **comparison, sources and next steps** are saved, follow [comparison-fidelity.md](comparison-fidelity.md), then run `python3 "$PEA_SKILL/scripts/save_gate.py" --project "$PEA_PROJECT" --scope comparison-full --evidence evidence.json --fidelity-document-id <registered-id> --json`. The older `--scope comparison` receipt checks only current journal/review/context revision and evidence hash; it cannot support the full wording. This does not certify a complete report, an available home, or permission to contact/book. For a completed report, register the snapshotted document, output and validated task using [state-sources-api.md](state-sources-api.md), then run the separate `--scope report --output-id <registered-output-id>` gate. That gate also requires the current report file to match its registered snapshot; an edited, missing or linked file needs a new valid registration. If either gate fails, describe what has been saved without saying the requested scope is complete; repair from current state before retrying, never replay the old user turn.
