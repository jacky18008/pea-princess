...
- **The point-source discount grades the area; it never erases the walk home.** A node sitting on the line from the station to the door counts in full, at full weight — one candidate had 89% of its counts on two nodes directly on the route home. Report `crime.night_route_node_share_flag` separately from the area tier.
