...
1. `python3 scripts/geo.py lookup "<postcode>"` then `python3 scripts/geo.py nearby --lat <lat> --lng <lng> --radius 400` and `python3 scripts/roads.py near --lat <lat> --lng <lng> --radius 400 --brief` — the nearest food shop, launderette, pharmacy and the walking distance to each.
