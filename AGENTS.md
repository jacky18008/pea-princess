...
4. `tools/session_runner.py` injects complete current conditions and only explicitly selected, revision-pinned source spans into model calls itself; file pointers alone do not ensure reading. Use bounded durable calls without automatic retry. Keep Claude paused unless the user changes that instruction.
