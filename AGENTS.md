...
2. At startup, after compaction, and before continuing sustained work, read `skills/vet-flat/references/session-harness.md`. If `.pea-state/events.json` exists, run `python3 skills/vet-flat/scripts/session_state.py --project . context --max-chars 96000`. Use the latest revision; an old summary/profile/report cannot override it. Stop dependent work if the packet is incomplete or invalid.
